    ...
    with open(args.events, "w") as f:
        ...
        try:
            for frame, pcm_frames in source.frames():
                ...
                if args.max_turns and turns_done >= args.max_turns and session.status == "listening":
                    break
                if not args.no_window:
                    shown = frame.copy()
                    draw_overlay(shown, session.tp.last_boxes, session.tp.last_track_ids, session.tp.last_provisional,
