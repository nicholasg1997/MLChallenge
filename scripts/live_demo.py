...
def main():
    ...
    parser.add_argument("--face-reader", default="head", choices=["head", "fusion"],
                        help="what the on-screen face reading (and the LM's face cue) comes from: the original face "
                             "encoder's own expression head (default; responds to posed expressions), or the fusion "
                             "model with text masked (what the MELD replay uses; prior-locked on a live face)")
    parser.add_argument("--face-threshold", type=float, default=None,
                        help="probability a non-neutral emotion needs before the face is read as it "
                             "(default 0.5 for --face-reader head, 0.25 for fusion)")
    parser.add_argument("--face-ema", type=float, default=0.5, help="per-sampled-frame smoothing of the head reading")
    ...
    face_reader = FaceEmotionEncoder(device=bundle.device) if args.face_reader == "head" else None   # original weights
    ...
    with open(args.events, "w") as f:
        ...
        session = LiveSession(bundle, bank_embeddings, transcriber, endpoint, emitter, responder=responder,
                              push_to_talk=args.push_to_talk, face_reader=face_reader,
                              face_threshold=args.face_threshold, face_ema=args.face_ema)
        warm_up(bundle, transcriber, responder, face_reader)
        ...
        print(f"ready: source={args.source} device={bundle.device} lm={args.model_repo if responder else 'off'} "
              ...
              f"face-reader={args.face_reader} (threshold {session.face_threshold})")
    ...
    report["face_reader"] = args.face_reader
    report["face_reader_params"] = count_parameters(face_reader.model)[0] if face_reader is not None else 0
